    ...
    class TreinosController
    {
        ...
        public function getSessaoParaRetomar($idSessao) {
            try {
                ...
                $resultado = $this->treinosService->getSessaoParaRetomar($idSessao);
                
                // Verificar se o usuário tem permissão para acessar esta sessão
                if ($resultado['sessao']['idUsuario'] != $usuario['sub'] || $resultado['sessao']['tipo_usuario'] != $usuario['tipo']) {
                    http_response_code(403);
                    echo json_encode(['success' => false, 'error' => 'Acesso negado a esta sessão']);
                    return;
                }
